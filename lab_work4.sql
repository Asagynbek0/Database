...
--PART 1
--TASK 1.1
SELECT first_name || ' ' || last_name AS full_name ,employees.department,salary employees;
--TASK 1.2
...
SELECT  project_name,budget,
        CASE
            ...
            WHEN budget BETWEEN 10000 AND 150000 THEN 'Medium'
            ELSE 'Small'
        END AS budget_category
FROM projects;

--1.4
SELECT  employees.first_name || ' ' || last_name AS employee_name,
...
--3.4
SELECT first_name || ' ' || last_name AS employee_name,EXTRACT(YEAR FROM AGE(CURRENT_DATE, hire_date)) AS years_with_company
FROM employees;
...
SELECT
    ...
    GREATEST(MAX(salary), 0) AS department_max_salary,
    LEAST(MIN(salary), 1000000) AS department_min_salary
FROM employees e1
GROUP BY department;